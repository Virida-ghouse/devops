...
# 🚁 Script de déploiement Gitea Runner via API Clever Cloud
# Ce script utilise l'API Clever Cloud directement pour contourner les problèmes de console
...
log "🚁 Déploiement Gitea Runner via API Clever Cloud"
...
if [ -z "$CLEVER_TOKEN" ] || [ -z "$CLEVER_SECRET" ]; then
    echo "Configuration des credentials Clever Cloud..."
    echo "Obtenez vos credentials sur: https://console.clever-cloud.com"
    echo "Ou utilisez: export CLEVER_TOKEN='votre_token' && export CLEVER_SECRET='votre_secret'"
    echo ""
...
# Token Gitea (à obtenir manuellement)
if [ -z "$GITEA_TOKEN" ]; then
    echo "Configuration Gitea..."
    echo "Obtenez votre token sur: $GITEA_INSTANCE_URL > Profil > Settings > Applications"
    echo ""
    
    read -p "Entrez votre token Gitea: " GITEA_TOKEN
    
    if [ -z "$GITEA_TOKEN" ]; then
        error "Token Gitea manquant"
        exit 1
    fi
fi
