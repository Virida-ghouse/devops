...
# 🚁 Script de déploiement automatisé Gitea Runner pour VIRIDA
# Ce script déploie automatiquement le Gitea runner avec les variables préconfigurées
...
log "🚁 Déploiement automatisé Gitea Runner pour VIRIDA"
...
if [ -z "$CLEVER_TOKEN" ] || [ -z "$CLEVER_SECRET" ]; then
    ...
    echo ""
    log "Ou obtenez vos credentials sur: https://console.clever-cloud.com"
    exit 1
fi
...
if [ -z "$GITEA_TOKEN" ]; then
    error "GITEA_TOKEN doit être défini"
    log "Définissez-le avec:"
    echo "export GITEA_TOKEN='votre_token_gitea'"
    echo ""
    log "Obtenez votre token sur: $GITEA_INSTANCE_URL > Profil > Settings > Applications"
    exit 1
fi
